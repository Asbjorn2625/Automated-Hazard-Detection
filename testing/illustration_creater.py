...
def display_point_cloud(point_cloud, title='Point Cloud'):
    ...
    vis.create_window(title, width=int(800*2), height=1000)
...
path = os.path.join(parent_dir, 'testing/first data set')
rgb_images = [os.path.join(path, img) for img in os.listdir(path) if
              img.startswith("rgb_image")]
...
for image in rgb_images:
    ...
    depth_filtered[np.where((depth > 1000) | (depth < 10))] = grey_color

    # Convert the depth image to a point cloud
    point_cloud = pp._depth_image_to_point_cloud(depth_filtered)
    display_point_cloud(point_cloud, title='Filtered Point Cloud')
    ...
    for i in range(5):
        ...
        angle_y = np.degrees(np.arccos(
            np.dot(normal_vector, np.array([1, 0, 0]))))  # Our camera is turned, so the floor is on the horizontal
        angle_x = np.degrees(np.arccos(np.dot(normal_vector, np.array(
            [0, 0, 1]))))  # the closer to zero the closer to being orthogonal to the cameras optical axis
    ...
    # Display the results
    #display_depth_image(depth, title='Depth Data')
    #display_depth_image(depth_filtered, title='Filtered Depth Data')
    #display_projected_points(img, plane_points_image, title='Projected Points on Image')
